...
import { PropertyType, PropertyStatus, UserRole, ListingType, ListingStatus, TransactionType, TransactionStatus } from '../../../core/entities';

export class DataGenerator {
  private static readonly SECTORS = [
    'Palermo', 'Recoleta', 'Puerto Madero', 'San Telmo', 'La Boca', 
    'Belgrano', 'Villa Crick', 'Caballito', 'Barracas', 'Nuñez',
    'Villa Urquiza', 'Colegiales', 'Almagro', 'Once', 'Constitución'
  ];
  ...
  static generateProperty(ownerId: string, tenantId?: string) {
    ...
    return {
      id: faker.string.uuid(),
      title: `${propertyType === PropertyType.HOUSE ? 'Casa' : 
               propertyType === PropertyType.APARTMENT ? 'Departamento' :
               propertyType === PropertyType.COMMERCIAL ? 'Local Comercial' :
               propertyType === PropertyType.LAND ? 'Terreno' : 'Depósito'} en ${faker.helpers.arrayElement(this.SECTORS)}`,
      ...
      price: basePrice,
      area: propertyType === PropertyType.LAND ? 
      ...
      bathrooms: [PropertyType.HOUSE, PropertyType.APARTMENT].includes(propertyType) ? 
        faker.number.int({ min: 1, max: 4 }) : null,
      parkingSpaces: faker.datatype.boolean(0.7) ? 
        faker.number.int({ min: 0, max: 3 }) : null,
      coordinates: `POINT(${longitude} ${latitude})`,
      valuation: faker.datatype.boolean(0.8) ? 
        basePrice * faker.number.float({ min: 0.9, max: 1.1 }) : null,
      tenantId: tenantId || faker.helpers.arrayElement(this.TENANT_IDS),
      ownerId,
      createdAt: faker.date.between({ from: '2020-01-01', to: new Date() }),
      updatedAt: new Date(),
  ...
  static generateListing(propertyId: string, tenantId?: string) {
    ...
    const basePrice = faker.number.int({ min: 
      listingType === ListingType.RENT ? 20000 : 50000, 
    ...
    return {
      ...
      expiryDate: faker.datatype.boolean(0.7) ? 
        faker.date.future({ years: 1, refDate: listedDate }) : null,
  ...
  static generateTransaction(propertyId: string, buyerId?: string, sellerId?: string, tenantId?: string) {
    const transactionType = faker.helpers.arrayElement(Object.values(TransactionType));
    const amount = faker.number.int({ min: 
      transactionType === TransactionType.RENT ? 15000 : 50000,
    ...
    return {
      ...
      status: isCompleted ? TransactionStatus.COMPLETED : 
      ...
      completionDate: isCompleted ? 
        faker.date.between({ from: transactionDate, to: new Date() }) : null,
      notes: faker.lorem.sentence(),
      metadata: {
        source: faker.helpers.arrayElement(['web', 'mobile', 'agent']),
        priority: faker.helpers.arrayElement(['low', 'medium', 'high']),
        tags: faker.helpers.arrayElements(['vip', 'urgent', 'negotiable', 'cash'], { min: 0, max: 2 })
      },
